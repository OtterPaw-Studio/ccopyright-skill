...
`init` 会创建 schema v3 事实模板和隐私安全的 `facts/requirements-snapshot.md`。
已有 v1/v2 工作区会原地迁移：保留现有事实和自定义门禁，补充新增字段，删除旧的
`portal_evidence` 附件历史模型，并将新增确认项保持为 `false`。
...
推测值不能自动确认。先按[门户表单校验配置](portal-form.md)逐项检查：名称/版本、
分类、原创或修改、开发方式、权利取得和范围、著作权人、完成与发表事实、六项环境、
语言、用途、行业、主要功能、技术特点、源码量和交存方式；再用
[当前官方来源](official-sources.md)及本次门户复核。最终模式还要求有序源码文件、
文档章节和全部必要确认。

`status` 会列出普通缺失项、条件分支缺失项、活动分支的证明准备缺口和门户字段约束问题。条件分支包括已发表、修改软件、继受取得、部分权利，以及合作/委托/下达任务开发。著作权人的证件号码、证件扫描件和签名不进入工作区。
...
选择程序或文档“例外交存”时，本 Skill 会停止；它只生成普通交存材料。合作开发、委托开发、下达任务开发、修改软件、继受取得和部分权利会在证明清单中动态出现相应项目。活动项目必须达到该分支允许的 `ready` 或经当前门户确认的 `not-required` 状态，才能最终生成；工具仍不会替申请人判断文件是否具有法律效力。
